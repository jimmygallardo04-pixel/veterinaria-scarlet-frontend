...
export default function NuevoArchivoPage() {
  ...
  const guardarArchivo = async () => {
    ...
    try {
      ...
      if (!res.ok) { toast.error("Error guardando en backend"); return; }
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <section className="card">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1">Fecha del documento *</label>
          ...
          <label
            ...
            onDrop={(e) => { e.preventDefault(); setDragActivo(false); seleccionarArchivo(e.dataTransfer.files?.[0] || null); }}
            className={`mt-4 flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed p-8 text-center transition-colors ${
              dragActivo ? "border-green-500 bg-green-50" : "border-slate-300 bg-slate-50 hover:bg-white"
            }`}
          >
          ...
          <textarea className="input mt-3" placeholder="Observaciones" rows={3}
